...
| 编号 | 功能 | 状态 |
|------|------|------|
| UI-TOP-01 | 顶部栏两个视图按钮（综合、车辆） | 已实现 |
| UI-TOP-02 | 当前视图高亮显示 | 已实现 |
| UI-TOP-03 | 仿真时钟显示 | 已实现 |
| UI-VW-01~04 | 综合视图：线路纵断面 + 列车动画 + 速度-位置曲线 + 状态卡片 | 已实现 |
| UI-VHC-01~03 | 车辆视图：速度-时间 + 加速度 + 工况指示器 | 已实现 |
| UI-CTRL-01~03 | 控制面板：运行/暂停/停止 + 倍率选择 + 单步 | 已实现 |
| UI-PARAM-01/02/05 | 参数编辑：车辆参数 + 线路参数 + 重置 | 部分实现（车辆已实现，线路区段选择待 Track 模块） |
| UI-EXPORT-01 | CSV 导出 | 已实现 |
| UI-BAR-01~04 | 状态栏：仿真时间 + FPS + 列车数 + 倍率 | 已实现 |
...
Generates commit messages in Chinese, Conventional Commits format. Subject ≤50 chars, hard cap 72. Imperative mood. Body only when the *why* is non-obvious or there are breaking changes. No AI attribution, no "this commit does X", no emoji unless the project uses them. Body always required for breaking changes, security fixes, data migrations, and reverts — future debuggers need the context.

Automatically stages changed files (`git add -A`), commits, and then **asks whether to push** to remote. Does not push without confirmation. Does not amend unless asked.
